...
def read_params():
    ...
    #################
	# arg( '-sl','--sample_select', type=str, help="define which samples must be included\n")
	arg( '-sl','--select', type=str, help="the samples to select\n")
	arg( '-rf','--remove', type=str, help="the samples to remove\n")
	arg( '-if','--include', type=str, help="the fields to include\n")
	arg( '-ef','--exclude', type=str, help="the fields to exclude\n")
    #################
...
if __name__ == "__main__":
	...
	f = pd.read_csv(par['inp_f'], sep='\t', header=None, index_col=0, low_memory=False ) #, dtype=unicode)
	f = f.T
...
# 	print(set(f["DATASET"]))
	...
	#if par['sample_select']:
		#onc_stag_vals = par['sample_select'].split(":")
		#print(par['sample_select'].split(":"))
		#if not len(onc_stag_vals) >= 2:
			#raise Exception("Here1")
		#if not onc_stag_vals[0] in f.columns:
			#raise Exception("Here2")
		#for el in onc_stag_vals[1:]:
			#if not el in set(f[onc_stag_vals[0]].values):
				#raise Exception("Here3")
		#print("Here")
		#print(f.shape)
		#f = f.loc[f[onc_stag_vals[0]].isin(onc_stag_vals[1:])]
		#print(f.shape)
	...
	pf2 = pd.DataFrame()
	if par['select']:
		pf2 = pf2.append(pd.DataFrame([s.split(':') for s in par['select'].split(',')],index=['select']*(par['select'].count(',')+1)))
	if par['remove']:
		pf2 = pf2.append(pd.DataFrame([s.split(':') for s in par['remove'].split(',')],index=['remove']*(par['remove'].count(',')+1)))
	if par['include']:
		pf2 = pf2.append(pd.DataFrame([s.split(':') for s in par['include'].split(',')],index=['include']*(par['include'].count(',')+1)))
	if par['exclude']:
		pf2 = pf2.append(pd.DataFrame([s.split(':') for s in par['exclude'].split(',')],index=['exclude']*(par['exclude'].count(',')+1)))
	...
	for i in range(len(pf2)):
		...
		if pf2.index[i] == 'select':
			f = f[f[pf2.iloc[i,0]].isin(pf2.iloc[i,1:])]

		if pf2.index[i] == 'remove':
			f = f[-f[pf2.iloc[i,0]].isin(pf2.iloc[i,1:])]

		if pf2.index[i] == 'include':
			if pf2.iloc[i,0] != 'feature_col':
				meta = [s for s in meta if s in pf2.iloc[i,0:].tolist()]
			else:
				feat = [s for s in feat if (pf2.iloc[i,1] in s) | ('unclassified' in s) ]

		if pf2.index[i] == 'exclude':
			if pf2.iloc[i,0] != 'feature_col':
				if pf2.iloc[i,0] == '_all_':
					meta = []
				else:
					meta = [s for s in meta if s not in pf2.iloc[i,0:].tolist()]
			else:
				if pf2.iloc[i,1] == '_all_':
					feat = []
				else:
					feat = [s for s in feat if pf2.iloc[i,1] not in s]
	...
	# print(f.head())
	# print(f.shape)
	# print([el for el in f.columns if "humann3_" not in el])
...
# 	raise Exception("Here999")
	...
	for j in range(runs_n*runs_cv_folds):
		...
		if lp.feature_selection == 'lasso':
			...
			alphas=lp.fs_grid[0], cv=lp.cv_folds, n_jobs=par['ncores']).fit(\
		...
		if lp.learner_type == 'rf':
			...
			clf.append(LassoCV(alphas=lp.cv_grid[0], cv=lp.cv_folds, n_jobs=par['ncores']).fit(\
				f.loc[i_tr[j] & i_u[j//runs_cv_folds], fi[j].feat_sel].values\
				, l[i_tr[j] & i_u[j//runs_cv_folds]].values.flatten().astype('int')))

		elif lp.learner_type == 'enet':
			clf.append(ElasticNetCV(\
			alphas=lp.cv_grid[0], l1_ratio=lp.cv_grid[1], cv=lp.cv_folds, n_jobs=par['ncores']).fit(\
			f.loc[i_tr[j] & i_u[j//runs_cv_folds], fi[j].feat_sel].values\
			, l[i_tr[j] & i_u[j//runs_cv_folds]].values.flatten().astype('int')))
...
# 	print("******************", l, i_tr_ob, i_u, len(feat), runs_n, runs_cv_folds)
